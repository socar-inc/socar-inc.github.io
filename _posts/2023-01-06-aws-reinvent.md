...
뿐만 아니라, 각 분야의 전문가와 각종 IT 관계자들이 모여 Best Pratice를 발표하고 공유하기도 합니다.

AWS가 제공하는 서비스 이외에도 다양한 솔루션 업체(GitLab, Redis, Docker, MariaDB 등등)이 모여 제공하는 서비스에 대한 소개를 들어볼 수 있는 컨퍼런스 입니다.
...
여기서 Aurora와 동일한 스토리지 구성이라는건 큰 의미가 있습니다.
Memory 기반의 데이터베이스인 Redis는 빠른 Return을 보장하지만 데이터에 대한 내구성이 떨어질 수 있습니다.
하지만 관계형 데이터베이스와 동일한 스토리지 구성을 가짐으로써 데이터의 손실이 발생하지 않도록 보장하며
기존의 Redis보다 데이터 안정성이 보장됩니다.
...
로컬 PC에서 VPN를 통한 RDS 접근 방법이나 쏘카 서비스 이용시 Load balancer부터 데이터베이스 구성까지 DB팀과 인프라팀의 업무는 긴밀하게 붙어있는 편입니다.
이런 과정에서 발생하는 인프라팀과 커뮤니케이션에 도움이 되는 세션이었습니다. 

### 2.3. SQL에서 NoSQL로 점진적으로 마이그레이션

이 세션은 관계형 데이터베이스에 적재된 데이터를 In-Memory기반의 데이터베이스에 데이터를 이관하는 방법을 소개했습니다.
...
실제 마이그레이션 작업에 못지않게 작업 대상에 대한 데이터의 전수조사도 중요하며, 파이썬 코드를 이용하여 update,insert,delete(DML) 작업을 구분하여 Dual Write 작업을 통해 이관을 합니다.
...
실제로 적용하기에는 비용 이슈가 발생할 수 있으며,운영에 신기능을 적용하기에는 어떤 버그가 발생할지 모르기 때문에 적용하기에는 시기상조라고 생각합니다.
...
쏘카에서는 S3에 적재된 데이터를 Athena를 이용하여 조회를 하지만, 이 외에도 여러 대안을 알고 있는 건 필요합니다.
...
특히 평소에 관심있는 분야에 대해 Deep Dive 하고 Insight를 가져볼 수 있는 값진 시간이었습니다.
