...
- [1. 아이콘 컴포넌트를 왜 개발하게 되었을까?](#1.-아이콘-컴포넌트를-왜-개발하게-되었을까?)
  - [1.1 기존 방식에 대해 알아보자](#1.1-기존-방식에-대해-알아보자)
  - [1.2 기존 방식은 어떤 문제가 있을까?](#1.2-기존-방식은-어떤-문제가-있을까?)
  - [1.3 이렇게 개선해보자](#1.3-이렇게-개선해보자)
- [2. 아이콘 컴포넌트, 어떻게 만들어졌을까?](#2.-아이콘-컴포넌트,-어떻게-만들어졌을까?)
  - [2.1 다양한 element 도형을 path element로 변경해보자](#2.1-다양한-element-도형을-path-element로-변경해보자)
  - [2.2 컴포넌트를 만들어서 적용해보자](#2.2-컴포넌트를-만들어서-적용해보자)
  - [2.3 타입을 적용해보자](#2.3-타입을-적용해보자)
  - [2.4 색상도 타입으로 정의해두면 어떨까~?](#2.4-색상도-타입으로-정의해두면-어떨까~?)
  - [2.5 스크립트 실행을 따로 따로 매번할 수 없잖아??](#2.5-스크립트-실행을-따로-따로-매번할-수-없잖아??)
  - [2.6 스토리북에 적용해서 확인해보자](#2.6-스토리북에-적용해서-확인해보자)
- [3. 무엇이 달라졌을까?](#3.-무엇이-달라졌을까?)
  - [3.1 기존의 문제점은?](#3.1-기존의-문제점은?)
  - [3.2 문제점이 해결 되었는가?](#3.2-문제점이-해결-되었는가?)
- [4. 추후 개선점](#4.-추후-개선점)
- [5. 마무리](#5.-마무리)

---

## 1. 아이콘 컴포넌트를 왜 개발하게 되었을까? <a name="1.-아이콘-컴포넌트를-왜-개발하게-되었을까?" />

쏘카의 디자인 시스템은 디자이너, Ios 개발자 Android 개발자, 웹프론트엔드 개발자가 협업하여 구축했습니다. 사용자가 일관된 UI/UX를 경험할 수 있도록 Ios, Android, 웹에서 모두 같은 형태의 컴포넌트들을 개발하여 사용합니다. 이번 글에서 다룰 아이콘 또한 디자인시스템에 포함되어있습니다.

쏘카에서 기존에 사용하던 방식은 아이콘을 웹폰트로 변환하여 사용하는 방식입니다. 해당 방식으로도 아이콘을 등록하고 사용하는 데에는 큰 무리가 없지만, 디자인시스템은 여러 개발자가 공동으로 사용하기 때문에 개발자 경험(Developer Experience, DX) 또한 중요합니다. 기존의 방식에는 개발자 경험에 문제가 있었기 때문에 아이콘 방식 개선에 대한 필요성을 느끼게 되었습니다.
...
기존에는 쏘카에서 아이콘을 폰트로 변환하여 사용하고 있었습니다. 기존의 방식을 먼저 설명드린 후에 어떤 문제점이 있었는지에 대해 말씀드리겠습니다.
...
2. 생성된 파일 등록
   ...
   <div style="display: flex; justify-content: space-between;">
       <img style="width: 60%; display: inline-block;" src="https://user-images.githubusercontent.com/72717402/177697181-5bb3f5f6-49e6-499f-82b5-0bb702a43c75.png" />
       <img style="width: 35%; display: inline-block;" src="https://user-images.githubusercontent.com/72717402/177697213-24b9d90c-1379-441c-b25f-c808b738c965.png" />
   </div>

   > 생성된 파일들을 디자인 시스템 저장소에 넣어줍니다. 이 때, 아이콘 폰트에 원하는 컬러를 주입할 수 있도록 하기 위해 css의 color 속성을 제거해줍니다.

3. 디자인시스템 배포
   - 아이콘 폰트 등록이 완료되었으면, 디자인시스템의 배포를 진행합니다. 디자인시스템을 사용중인 모든 서비스들도 디자인시스템 버전을 최신화해서 같이 배포를 진행합니다.
...
### 1.2 기존 방식은 어떤 문제가 있을까? <a name="1.2-기존-방식은-어떤-문제가-있을까?" />
...
### 2.1 다양한 element 도형을 path element로 변경해보자 <a name="2.1-다양한-element-도형을-path-element로-변경해보자" />
...
### 2.2 컴포넌트를 만들어서 적용해보자 <a name="2.2-컴포넌트를-만들어서-적용해보자" />
...
### 2.3 타입을 적용해보자 <a name="2.3-타입을-적용해보자" />
...
### 2.4 색상도 타입으로 정의해두면 어떨까~? <a name="2.4-색상도-타입으로-정의해두면-어떨까~?" />
...
### 2.5 스크립트 실행을 따로 따로 매번할 수 없잖아?? <a name="2.5-스크립트-실행을-따로-따로-매번할-수-없잖아??" />
...
### 2.6 스토리북에 적용해서 확인해보자 <a name="2.6-스토리북에-적용해서-확인해보자" />
...
## 3. **무엇이 달라졌을까?** <a name="3.-무엇이-달라졌을까?" />

### 3.1 기존의 문제점은? <a name="3.1-기존의-문제점은?" />
...
### 3.2 문제점이 해결 되었는가? <a name="3.2-문제점이-해결-되었는가?" />
...
## 4. 추후 개선점 <a name="4.-추후-개선점" />

- [SVGR](https://react-svgr.com/docs/webpack/) 을 활용해 svg 파싱
...
쏘카 웹 프론트엔드 팀에서 리액트 아이콘 컴포넌트를 개발한 과정에 대해 알아보았습니다. 외부 서비스는 사용자 경험을 중요시하는 것처럼, 회사 내 개발자들이 사용하는 서비스는 개발자 경험이 중요합니다.

컴포넌트는 한번 개발하면 여러 개발자들이 공동으로 사용하게 되기 때문에 이번 개발의 가장 큰 목적은 '개발자 경험 향상'이었습니다. 팀원들이 이번에 바뀐 아이콘 등록 방식과 컴포넌트를 잘 사용하고 있는 걸 보면 목적을 잘 달성했다고 생각이 듭니다.

![image](https://user-images.githubusercontent.com/72717402/178921491-f6eaeeb2-86fa-4ef4-bf50-68dbebe9455d.png)
