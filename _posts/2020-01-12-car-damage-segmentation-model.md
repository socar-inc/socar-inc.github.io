...
> 1. 사용자가 차량을 인수할 장소를 선택하게 됩니다.
>
> 2. 운행을 원하는 시간을 선택하고,
>
> 3. 운행을 원하는 차량을 선택하게 됩니다.
>
> 4. 약속한 운행 시간이 가까워지면, 선택한 장소에서 차량을 인수 받고 상태를 확인합니다.
>
>    - 이 때 사용자는 앱 내에 인수 받은 당시의 차량의 외관 사진을 업로드할 수 있습니다.
>
...
문제는 고객이 쏘카를 운행 중, 차량을 파손시킬 때 발생합니다.

쏘카는 고객에게 차량을 서비스하는 회사이고, 따라서 차량은 쏘카의 가장 중요한 자원입니다. 이러한 차량의 파손 상태를 꾸준하게 모니터링 하는 것은 쏘카가 반드시 해야할 일 중 하나입니다.

쏘카는 모니터링을 위해 고객이 차량 인수 후 상태 확인 과정에서 직접 업로드하는 차량의 외관 이미지를 사용하고자 했습니다. 이 데이터는 파손에 대한 여러가지 히스토리를 가장 잘 제공할 수 있는 데이터라고 판단했기 때문입니다.
...
하지만 기존의 업무 프로세스 내에서는 차량 외관 이미지를 업무 인력이 직접 검수하는 과정을 거쳤고, 이 작업은 회사 차원에서 굉장한 업무 부담으로 다가올 수 밖에 없었습니다.

 * 일 평균 7-8만장, 최대 11만장의 차량 외관 이미지가 업로드되기 때문입니다. 검수할 분량이 너무 많았습니다.

사실상 검수 작업에 리소스를 할당하지 못하는 상황이 되었고, 차량의 파손 상태를 모니터링하는 용도보다는 파손 신고 접수 시, 업무 인력이 직접 해당 차량의 외관 이미지를 현재로부터 과거로 추적해가며 파손 시점을 찾아내는 데에만 사용되는 데이터가 되었습니다.
...
### 어떻게 이러한 문제를 해결해야 할까?
그럼에도 불구하고, 차량 외관 이미지를 단순히 추적 시 일부만 사용되는 데이터로만 남겨두는 것에는 아쉬움이 컸습니다. 결국 차량 외관 이미지를 적극적으로 활용하지 못한 이유는 업무 리소스를 할당하기에 부담이 크기 때문인데, 반드시 검수 작업에 업무 인력이 할당되어야 하는가에 대한 의문이 들었습니다.

- '반드시 사람이 눈으로 봐야만 검수를 할 수 있는 걸까?'

- 이러한 고민들 끝에, 딥러닝 모델을 이용한 차량 파손 탐지 자동화 프로젝트를 진행하게 되었습니다.

- <span style="color:red">주어진 차량 이미지 내의 파손 영역과 파손 종류를 자동으로 판단하는 딥러닝 모델</span>을 구현하는 것을 프로젝트의 목표로 설정했습니다.
...
가장 먼저 정의해야 했던 것은, 이 프로젝트가 어떤 태스크로 분류될 수 있는지에 대한 점이였습니다.
...
**3. Object Detection (객체 검출)**

- 일반적으로 Classification과 Localization을 동시에 수행합니다.
- 입력으로 주어진 이미지 안의 객체의 위치(localization)와, 해당 객체의 종류(classification)을 출력하는 행위입니다.
...
이러한 점들을 고려했을 때, 모델이 수행해야 하는 행위를 기술적으로 분류 시, **Semantic Segmentation**에 속한다고 설정할 수 있었습니다.
...
입력 데이터는 분석의 대상이 되는 데이터로, 알파벳 *X*로 표기합니다. 이는 독립변수, 설명변수라는 용어로 불리우기도 하며, 해당 포스트에서는 Feature라는 용어로 표현하겠습니다.

출력 데이터는 문제 해결을 위해 예측하고자 하는 목적 데이터로, 알파벳 *Y*로 표기합니다.이는 종속변수라는 용어로 불리우기도 하며, 해당 포스트에서는 Label 또는 Target이라는 용어로 표현하겠습니다.

해당 프로젝트에서 데이터 측면에서 정의가 필요한 점들은 다음과 같았습니다.

- 이미지 단위의 파손 존재 여부 (이미지 전체에서 파손이 존재하는지에 대한 True/False)
- 픽셀 단위의 파손 클래스 분류
  - 스크래치 (Scratch) / 찌그러짐 (Dent) / 이격 (Spacing) / 해당 없음 (Background)

따라서 입력 데이터(Feature)는 차량 이미지가 될 것이며, 출력 데이터(Label)는 이미지 단위 파손 존재 여부와 픽셀 단위 파손 클래스 분류가 될 수 있겠습니다.

(외부 레이블링 업체의 도움을 받아), 사용자가 쏘카 앱 내에서 업로드한 이미지 중 육안으로 파손의 여부를 확실하게 판단할 수 있는 이미지 2,000장에 대해 Label을 정의했고, 형식은 다음과 같습니다.

- image_id: 이미지 파일명
- damage_level: 이미지 단위 파손 존재 여부. 파손이 존재할 경우 1, 파손이 존재하지 않을 경우 0
- regions: 파손 영역 단위 파손 클래스 분류
  - 데이터 정의 시 데이터 관리의 용이함을 위하여 픽셀 단위로 정의하는 mask 방식이 아닌 영역 단위로 정의하는 polygon 방식을 채택하였으며, 학습 시 해당 polygon 정보를 픽셀 단위 mask로 변환하여 이용하였습니다. 
  - type: 데이터 정의 방식 (polygon)
  - points: 해당 파손 영역의 외곽선좌표 배열
  - class_name: 해당 파손 영역의 종류 
    - 0: 파손 해당 없음 / 1: 스크래치 / 2: 찌그러짐 / 3: 이격
...
이렇게 정의된 데이터를 이용 목적에 따라 분리하는 작업을 진행했습니다. 

전체 2,000개의 데이터를 학습용 데이터(Training set), 검증 및 모델 선택용 데이터(Validation set), 실제 테스트용 데이터(Test set)으로 나누었고, 그 비율은 8:1:1로 설정하였습니다.

데이터셋 분리 시, 특정 용도의 데이터셋에 어떠한 특징이 편향되는 것을 막기 위하여 (예를 들면, 찌그러진 파손에 해당하는 이미지 대부분이 Validation set에만 포함되어 있어 학습 시 해당 특징이 모델에 충분히 반영되지 못하는 문제점) Stratified split을 채택하여 진행했습니다. 

Stratified split이란 데이터셋 분리 시 데이터 분포가 편향되지 않도록 분리하는 방법으로, 해당 프로젝트에서는 파손 클래스와 전체 이미지 면적 대비 파손 영역이 차지하는 면적 비율이 편향되지 않도록 설정하여 Training set, Validation set, Test set으로 분리했습니다.
...
- U-Net의 장점
  ...
  1. 빠른 속도
     ...
     - 기존 Segmentation 모델의 단점이였던 느린 연산 속도를 개선하였습니다.
  ...
  2. Context와 Localization의 늪에서 탈출
     ...
     - Patch의 크기가 커지면 더 넓은 범위의 이미지를 한번에 인식할 수 있게 되어 Context 파악에는 탁월한 효과를 보이지만, 많은 Max-pooling을 거치며 Localization 성능에는 부정적인 영향을 미치게 됩니다.
...
- U-Net with EfficientNet
  ...
  1. Contracting path
     ...
     Contracting path는 Encoder의 역할을 수행하는 부분으로 전형적인 Convolution network로 구성되어 있습니다.

     Contracting path는 입력을 Feature map으로 변형함으로서 이미지의 Context를 파악합니다.

     이 때, Contracting path의 앞단에 이미 잘 학습된 모델을 일종의 Backbone으로 사용함으로서 학습 과정에서의 효율과 성능을 높일 수 있습니다. 주로 ResNet 등의 모델을 사용하곤 합니다. 해당 프로젝트에서는 ImageNet 데이터셋으로 학습된 **EfficientNet**을 Contracting path의 Backbone으로 사용했습니다. 

  2. Expanding path

     Expanding path는 Decoder의 역할을 수행하는 부분으로, 전형적인 Upsampling+Convolution network로 구성되어 있습니다. 즉, Convolution 연산을 거치기 전 Contracting path에서 줄어든 사이즈를 다시 복원하는(Upsampling) 형태입니다. 

     Expanding path에서는 Contracting을 통해 얻어진 Feature map을 Upsampling하고, 각 Expanding 단계에 대응되는 Contracting 단계에서의 Feature map과 결합함(Skip-connection concatenate)으로서 더 정확한 Localization을 수행하게 됩니다.

     즉, Multi-scale object segmentation을 위하여 Downsampling과 Upsampling을 순서대로 반복하는 구조로 되어있습니다.

     

- U-Net을 선택한 이유

  모델 선택 과정에서 고려 대상이 되었던 모델은 Sementic segmentation 분야에서 가장 널리 사용되고 있는 U-Net 모델과 DeepLab V3 모델이였습니다.

  일차적으로 두 모델의 성능을 정량적으로 나타내고 싶었으나, 각 모델이 개발된 목적이 달랐고 그에 따라 공개 논문에 성능 평가를 위해 사용된 지표가 일치하지 않아 (U-Net 모델의 경우 의학 이미지 Segmentation을 주 목적으로 개발된 모델이고, 그에 따라 일반적인 논문에서 사용되는 Pascal VOC 2012, COCO 등의 지표가 아닌 EM segmentation challange에서의 스코어를 기재함.) 정량적인 표현이 어려운 상황이였습니다.
  ...
  베이스라인 성능 비교 결과 DeepLab v3의 경우 mIOU 80.7, U-Net의 경우 mIOU 92.2를 기록했습니다.

  위의 결과를 통해 해당 프로젝트에서 사용하게 될 데이터의 경우 U-Net을 이용한 Semantic segmentation이 더 효과적이라고 판단하였고, 추후 개발 과정에 U-Net을 사용하게 되었습니다.
...
<img src="https://www.dropbox.com/s/jheqse4l2d6zldw/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202020-01-06%2019.10.55.png?dl=1" style="zoom:50%;" />

- Feature를 입력으로, 모델의 각 계층(Layer)의 파라미터와의 연산을 통해 예측값을 도출합니다. (Prediction)
- 모델이 예측한 값과 본래의 답안지(Target)가 얼마나 차이가 있는지 (Loss) 계산합니다.
...
- Feature는 차량 이미지, Target으로는 마스크(Binary mask image)가 사용됩니다.
  ...
  - 가공 방법은 다음과 같습니다.
    ...
    - 파손 영역별로 분리된 좌표 형태를 마스크 형식으로 변환합니다. 이 과정을 통해 각 파손 영역별로 한 장의 2차원 마스크를 가지게 됩니다. 
...
- 이 예측 마스크는 Mask Policy에 의해 0과 1의 이진 마스크로 변환되게 됩니다. 0과 1 사이의 연속적인 확률값을 속한다, 속하지 않는다의 이산적인 클래스 정보로 변환하는 과정을 거치는 것이죠.
  ...
  - 해당 프로젝트에서 고안해 사용한 Mask Poilicy는 각 파손 클래스 별 특성을 반영한 규칙이였습니다.

    해당 모델이 다루게 된 파손의 종류는 스크래치, 찌그러짐, 이격 총 3가지인데, 이 파손들의 속성이 서로 다르다고 생각했습니다.

    스크래치와 이격의 경우, 세밀한 영역 경계 설정이 필요하고, 찌그러짐의 경우 비교적 넓은 면적으로 발생할 가능성이 높은 파손입니다.

    따라서, 스크래치와 이격의 경우는 세밀한 경계 설정이 가능한 CRF 후처리 방식을 이용해 확률 마스크를 이진 마스크로 변환하였고, 찌그러짐의 경우 특정 확률값(Threshold value, 일반적으로 0.5을 기준선으로 사용) 이상의 값을 가지는 픽셀은 1로, 나머지 픽셀은 0으로 변환하는 thresholding 방식을 사용했습니다.

    각 파손 클래스마다 서로 다른 변환 정책을 적용함으로서 보다 각 파손의 속성이 반영된 이진 마스크를 생성할 수 있었습니다.

- 이렇게 변환된 이진 마스크를 이용해 파손 영역의 좌표를 구하고, 입력 이미지 위에 외곽선(Polyline)을 덧그려 이미지 검수 관리자가 확인하기 용이한 형태로 출력을 마치게 됩니다.
...
학습을 완료한 모델을 실제로 쏘카 앱을 통해 업로드되는 이미지들을 이용해 검증한 결과, 몇가지 보완해야할 점들을 발견했습니다.
...
1. **모델의 정확도에 대한 문제**
   ...
   - 첫 번째 문제점은 앱 내에서 차량 외관 촬영 시 가이드라인이 존재하지 않아 사용자들이 사진을 촬영하는 방식이 제각각이라는 것이였습니다. 

     멀리서 찍어 차량이 아닌 주차선이 이미지의 대부분을 차지하거나, 차량 루프를 촬영하는 과정에서 뒤편에 주차된 차량들이 빼곡히 촬영되거나, 뒤편의 건물이 촬영된 이미지들이 적지 않게 있었습니다.

   - 두 번째 문제점은 어두운 곳에서 촬영된 이미지에 취약점을 보인다는 점이였습니다.
...
2. **모델 측면에서의 보완 방법**
   ...
   - 모델의 연산량으로 인한 처리 속도 저하 해결을 위해서는 Global damage classifier을 추가하는 방법을 택했습니다. 실제로 쏘카 앱을 통해 업로드되는 이미지 중, 손상이 존재하는 이미지는 소수인데 모든 이미지를 깊은 Segmentation network에 통과시킬 필요가 있냐는 의문에서 시작된 아이디어였습니다.
...
- 모델의 정확도
  ...
  Global damage classifier의 분류 성능: 96%의 Accuracy, 96%의 F1-score을 기록하였습니다.
