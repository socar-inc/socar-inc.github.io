...
dbt는 SQL만으로 데이터 파이프라인을 구성할 수 있게 해주는 오픈소스 툴입니다. dbt는 데이터 엔지니어링의 큰 요소 중 하나인 ETL or ELT (Extract, Transform, Load) 중 **변형(Transform)에 집중합니다**. 어디서 데이터를 추출해내고 적재하는 지에 대한 관심보다는 “어떻게" 존재하는 데이터를 변형해서, 재사용할지에 대해 고민합니다.
