...
데이터 디스커버리를 가능하게 하는 플랫폼입니다. 주로 웹 형태로 제공되며, 데이터의 구조와 관계 등 메타데이터를 한 곳에서 쉽게 보고 검색할 수 있습니다. (같은 이유로, 메타데이터 플랫폼 이라는 용어와 혼용되기도 합니다.) 예시로는 다음이 있습니다. 
...
* Amundsen 은 python script 를 실행하여 메타데이터를 주입합니다. 
...
![datahub-ingestion-script](/img/data-discovery-platform-01/datahub-ingestion-script.png)*Datahub의 Bigquery 데이터 주입 script*
...
![datahub-dataset](/img/data-discovery-platform-01/datahub-dataset.png) *데이터허브 상세 UI*
...
dbt 는 데이터 ETL 과정에서 T(Transform) 과정을 효율화하는 도구입니다. dbt 를 이용하면 SQL 쿼리 모듈화, 테스트, 계보 확인을 편하게 할 수 있습니다. 데이터 엔지니어링 그룹 내에서 dbt 를 PoC 하고 있었기 때문에, 이 측면에서 Datahub 가 좀더 매력적으로 보였습니다. 
...
Data Discovery에서는 데이터 사용자의 이용 패턴도 중요한 요소 중 하나로 꼽힙니다. Datahub 는 시각화된 이용 분석 페이지가 따로 존재하여 자주 검색된 데이터셋  / 자주 수행된 액션 등을 그래프로 확인 할 수 있었습니다. Amundsen 은 따로 분석 페이지는 없었으나 메인 화면에서 "인기 있는 데이터셋"을, 각 테이블마다 "해당 테이블을 자주 쓴 사람"을 확인할 수 있었습니다.
...
데이터 이용자 측면에서는 위에서도 비교했듯이, Datahub 가 문서화, 오너십, 권한, 통계, 데이터 계보 관점에서 더 다양하고 풍부한 기능들을 지원합니다. 이런 기능들이 실제로 도입됐을 때, 이용자가 원하는 데이터를 빠르게 찾고 쏘카의 데이터 디스커버리를 발전시키는 데에 더 많은 도움을 얻을 수 있을거라 판단했습니다.
...
```
고민 : 3.비교분석 부분에 이상적으로는 매 항목마다 스크린샷을 넣고 싶은데, 그렇게 하면 글이 너무 길어지고 스크린샷이 차지하는 비중이 너무 많아지는 것 같아 고민입니다. 어떻게 하는게 좋을까요?(지금처럼 가끔씩 넣는다 / 글이 길어지더라도 모든 항목에 스크린샷을 넣는다)
```
