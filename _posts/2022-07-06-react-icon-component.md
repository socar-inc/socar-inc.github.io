...
차례로 해석해보면 M 2,12는 (2,12)에서 시작하여 차례로 점이 찍혀 원을 만들게 됩니다. 알파벳 C는 curve 명령어로 각 점들을 곡선으로 잇게 됩니다. C (x,y), (x,y), (x,y) 이렇게 세개의 좌표가 찍히는데 마지막 좌표는 끝점을 가리키고 앞의 두 좌표는 첫 점과 끝점을 잇는, 곡선을 만드는 점이라고 생각하면 됩니다.
M2,12 C 2,9.7909, 3.7909,8, 6,8 시작점 (2,12)에서 끝점 (6,8)을 잇는데 중간 다리 역할을 하는 것이 (2,9.8), (3.8,8) 이라고 해석할 수 있습니다.
C 8.2091,8, 10,9.7909, 10,12 시작점 (3.8,8)에서 끝점 (10,12)를 잇는데 중간 다리 역할을 하는 것이 (8.2,8), (10, 9.8) 이라고 해석할 수 있습니다. 이런 식으로 모든 점을 그려서 잇다보면 circle로 그렸던 원 아이콘을 얻을 수 있습니다.
...
`Object.key()`를 활용해 아이콘 이름을 가져오는 스트립트 파일을 실행했을 때 나오는 결과물입니다. (?? 어색...)

![0](/img/icon-component/iconType.png)

아이콘 이름을 type으로 정의하여 적용하니 컴포넌트를 사용할 때 아이콘 목록이 뜨는 것을 확인할 수 있었습니다.
![1](/img/icon-component/iconName.png)
...
색상코드가 적힌 scss 파일을 문자열 메소드를 이용하여 색상 이름만 가져와 json 파일을 만들어 줍니다. (// 내용 추가..?)
![2](/img/icon-component/iconPaletteCss.png)
![3](/img/icon-component/iconPalette.png)

색상 값을 추가하거나 변경이 필요하다면 scss 파일을 수정하고 스크립트 파일을 실행시키면 해결됩니다.
...
![4](/img/icon-component/storybookTest.png)

이렇게 아이콘과 색을 지정해서 출력해볼 수 있으며, 디자이너가 원하는 아이콘이 맞는지 QA도 진행할 수 있습니다.
![5](/img/icon-component/storybook.png)
...
1. svg 파일을 웹폰트로 변경할 때 외부 서비스에 의존해야하는 점
2. 생성된 파일들(폰트파일에 css파일, json 파일)을 덮어씌워줘야 하는데, 하나라도 빼트리거나 잘못 덮어씌우면 아이콘이 제대로 나오지 않는 점
3. 배포 이후에 아이콘을 사용하고 있는 서비스에서 일일이 디자인시스템 버전을 올려줘야하는 점 **-> 이거 설명 듣고 다시 작성..**

### 4.2 문제점이 해결 되었는가?

자신 있게 문제점이 100% 해결됐다고 말할 수 있습니다.
svg 파일을 사용하게 되어 1,2번이 동시에 해결 됐습니다.
스크립트를 실행함으로서 외부 서비스를 사용하지 않아도 되고, 있는 그대로의 파일을 사용하면 되기 때문에 덮어씌워주는 번거로움도 없게 됐습니다.
아이콘을 업데이트하기 위해서는 여러 단계를 거쳐야 했었는데 이제는 명령어 한 줄만 입력하면 깔끔하게 업데이트 되는 것을 볼 수 있습니다.
