...
b. Queue를 통해 전달하게될 메시지 구조만 협의되면. 이후 작업은 메시지를 Push 하거나 Pull 하기만 하면 됩니다. 본 시스템의 경우 모델에 판정을 의뢰할 외부 시스템은 판정 대상 이미지의 URL과 기타 정보를 메시지로 묶어 SQS에 push하면 되고, 판정 모델 쪽은 SQS를 polling 하면서 요청된 메시지를 받아와 판정 작업을 처리하고 결과를 업데이트 하면 됩니다.<br>
