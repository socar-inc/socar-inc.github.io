...
* 구현 가능한 시나리오
  * 일반적 시나리오
    ...
    2. ACC1 단독: 2.1A 소모
    3. ACC2 단독: 7.5A 소모
    4. ACC1 + Light 1단 (흔한 유형): 2.9A 소모
    5. ACC2 + Light 2단 (흔한 유형): 16.3A 소모
    6. ACC2 + 송풍기 2단 (흔히 말하는 차박): 11.7A 소모
  * 최악의 시나리오
    * ACC2 + 실내등2개 (문 앞뒤열림) + 송풍기 4단계 + Light 3단 + 비상등 +핸드폰 충전기: 32.7A 소모
...
1. 시나리오 별 회생 불능 방전에 도달하기까지 걸리는 시간
   ...
   * 시나리오 별, 실 평균 용량 기준 버틸 수 있는 시간의 정의: 정격 용량의 절반에 도달하기까지 걸리는 시간으로 이 이하의 영역에서는 배터리 상태 복원이 물리적으로 어려워짐.

   * | S#-Worst[hrs] | S#1[hrs] | S#2[hrs] | S#3[hrs] | S#4[hrs] | S#5[hrs] | S#6[hrs] |
     | ------------- | -------- | -------- | -------- | -------- | -------- | -------- |
...
2. 시나리오별 빈도 (과거 사례 기반, 순서에 따라 정렬함)
   ...
   2. S#5
   3. S#4
3. 고객이 차량 정차 후 정차된 차량으로 돌아오기 어려울 정도로 멀리 떠나기 까지 걸리는 시간 → 보통 10~15분 (Call 분석)
