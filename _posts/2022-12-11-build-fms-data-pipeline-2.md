...
> 위에서 소프트웨어 테스트와 데이터 퀄리티 테스트를 소개하고 있어서 이 부분 뒤에 데이터 퀄리티 테스트에 대한 내용이 소개되어야 할것 같은 느낌.... 
> 소프트웨어 테스팅 내용을 좀 줄여서 1.1 에 녹이는 것이 어떨까요? 
...
데이터 무결성은 데이터가 일관되고 정확해야 함을 의미합니다. 예를 들어 A 테이블의 B 컬럼은 Non Null이어야 하며 C 컬럼은 비즈니스 규칙에 따라 값의 범위가 0~100이어야 하는 등의 조건이 있다면, 특정 레코드가 이를 어겼을 때 무결성이 깨졌다고 이야기합니다.

FMS 프로젝트에서는 데이터 정합성, 무결성 검사를 함께 진행하였습니다.
