...
작업한 모델을 실제 서비스에 붙이기 위해서는 여러 인프라 작업이 필요했습니다.
...
예약 테트리스 프로젝트의 성과를 확인하기 위해서 일별 유휴차량비율 대비 가동률을 적용 전후로 확인해보았습니다. 
이때 유휴차량비율이란 하루 동안 운영된 전체 차량 중 예약이 하나도 없던 차량의 비율을 가리키는 지표입니다.

![img](/img/reservation-tetris/result-graph.png){: width="90%"}

그래프에서 볼 수 있듯이 적용 후의 추세선이 전과 비교해 더 높은 것을 확인할 수 있습니다. 
이는 동일한 유휴차량비율 관점에서 보면 같은 대수의 차량을 공급하여 더 많은 예약을 수용할 수 있다는 것을, 동일한 가동률 관점에서는 더 적은 비용으로 동일한 매출을 만들어낼 수 있음을 의미합니다.
...
![img](/img/reservation-tetris/reaction.png){: width="90%"}


## 마무리

실시간으로 서비스에 적용되는 최적화 프로젝트는 처음 진행해 본 거라 시행착오도 있었지만 함께 작업하며 서포트 해주신 많은 분들 덕분에 성공적으로 프로젝트를 마무리 할 수 있었다고 생각합니다.
예약 테트리스 프로젝트에서 서버 개발 쪽을 맡아주신 맷과 브루스, 프로젝트가 진행되는 동안 여러 부서를 오가며 디테일하게 챙겨주신 주디, 인프라 작업과 제 멘탈 지킴이로 큰 도움을 주신 카일, 그리고 적극적으로 피드백 주신 제주사업팀과 기타 지역사업팀에게 감사드립니다.
