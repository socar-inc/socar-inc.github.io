...
이번 글에서는 쏘카 데이터 그룹에서 처음 Airflow를 사용한 시점(2018년)부터 현재(2021년)동안 어떻게 환경을 구축했고 운영했는지를 소개합니다. 특히 최근에쏘카 데이터 그룹에서 Kubernetes(정확히는 Google Kubernetes Engine)를 도입했는데, 이 환경에서 Airflow를 활용한 내용을 자세히 담았습니다. 

이 글을 통해 다음과 같은 내용을 파악할 수 있으며, 어떤 상황에 어떤 판단을 했는지 작성했으니 이 글을 보시는 분의 상황에 맞게 선택해볼 수 있게 글을 구성했습니다. 필요한 분을 위해 코드 스니펫도 추가했습니다.

- Apache Airflow의 이해
- Managed Service : Google Cloud Composer
- Airflow 운영 전략
...
데이터 그룹이 처음 설립된 2018년엔 전사 공용 스케줄링 도구로 [Rundeck](https://github.com/rundeck/rundeck)을 사용하고 있었습니다. Rundeck은 스케줄링 기능을 제공해 배치 작업을 할 수 있는 업무에서 활용되고 있습니다. 

쏘카 서비스의 작업에서 활용하고 있는 Rundeck을 사용할지 혹은 새로운 도구를 도입할지 고민한 결과, 새로운 도구를 도입하기로 결정했습니다.

그 당시 이런 선택을 하기 전에 필요한 요구 조건을 정리했습니다.

1. 데이터 엔지니어링용 스케줄링 작업과 서비스의 작업 분리
2. 데이터 엔지니어링을 위한 편의 기능(클라우드의 특정 서비스의 추상화된 기능) - 쏘카는 데이터 웨어하우스로 빅쿼리를 사용하고 있으므로, 빅쿼리 관련 기능을 찾아봤습니다.
3. Workflow의 시각화 기능 강화

- TBU: 우민에게 왜 쓰게 되었는지 물어보기. 저도 시간이 지나서 기억이 살짝 가물가물하네요

## 태동기 - Google Cloud Composer

지금으로부터 약 2년 전인 2019년 이야기입니다. 데이터 그룹의 태동기로, 이 때는 데이터 엔지니어 없이 데이터 분석가들이 직접 데이터 파이프라인을 만들었습니다. 데이터 엔지니어가 부족하기 때문에, 데이터 분석가분들이 직접 데이터 파이프라인을 구축했습니다. Apache Airflow를 이미 사용해본 경험이 있는 분석가가 계셨고, 운영을 쉽게 가능하도록 구글 클라우드의 Cloud Composer를 사용하기로 결정했습니다. Cloud Composer는 구글 클라우드에서 제공하는 Managed Airflow입니다. 참고로 현재 AWS에도 Managed Airflow인 MWAA가 존재하지만, 그 당시엔 구글 클라우드의 Cloud Composer가 유일했고, 쏘카에선 데이터 웨어하우스를 구글 클라우드의 빅쿼리로 사용해서 이런 선택을 했습니다.
...
**그러나 Composer는 종종 알 수 없는 에러를 발생했습니다.** 당시 Airflow의 버전은 1.10.3로 오픈소스 자체에도 버그가 존재했습니다. 1.10.3 버전의 가장 큰 문제는 오류가 생겼을 때 로그를 직접 제대로 볼 수 없었습니다. Composer의 안정성 문제와 로그와 관련된 이슈는 주요 사항으로 떠올랐고, 이 부분을 해결하기로 했습니다.

<br>

## 초창기 - Google Compute Engine + Docker Compose

데이터 그룹이 생긴지 몇 달 뒤, 데이터 엔지니어 한 분 두 분 조직에 그룹에 합류하게 되었고, 자체 데이터 엔지니어링 팀이 생겼습니다. 데이터 엔지니어링팀의 핵심 역할 중 하나는 데이터 파이프라인을 구축하는 것이고, 다른 핵심 역할은 데이터 인프라 운영이였습니다. Cloud Composer를 벗어나 데이터 엔지니어링 팀에서 Airflow를 셋팅하고 운영하기로 결정했습니다.

<br>

### 고려 사항

이 때, 우선적으로 고려한 사항은 다음과 같습니다.
...
고려 끝에 선택한 배포 방법은 Google Compute Engine 위에 Docker Compose로 Airflow의 각 컴포넌트(웹 서버, 스케쥴러 등)를 직접 도커 컨테이너로 띄우는 방법입니다. Github에 있는 [puckel/docker-airflow](puckel/docker-airflow) 이미지를 이용하면 이를 쉽게 구현할 수 있었습니다.  
또한 분석 팀원들에게 익숙한 Jupyter Notebook을 동일 환경에서 배포하고, Airflow DAG을 수정할 때 Jupyter Notebook을 사용하도록 안내했습니다.
<br>
...
- 2-1) 여러 사람들이 작성한 DAG들은 모두 제각각의 스타일로 작성되었습니다. 예를 들어 어떤 DAG에는 `on_failure_callback` 값이 있는 반면, 어떤 DAG들은 이 값들이 보이지 않았습니다. 
- 2-2 )Jupyter Notebook 환경에서 DAG 코드를 작성하다보니 PEP8과 같은 스타일 컨벤션을 잘 지키긴 어려웠습니다. 
- 2-3) 가장 큰 문제는 DAG 코드가 담긴 모듈 이름에 규칙이 없어서, Airflow 웹에서 문제가 생긴 DAG이 실제로 어떤 파일인지 찾기 쉽지 않았습니다. 
- 2-4) DAG 파일을 언제 생성했고 수정했는지에 대한 히스토리가 없어서 문제가 생겼을 때 원인을 찾기가 어려웠습니다.  
...
그리고 테스트하는 과정에서 실제 운영 중인 테이블에 직접 접근하는 로직이 포함된 DAG이 있는데, 이 DAG들이 제일 큰 문제였습니다. 예를 들어 DAG 작성자가 `BigqueryOperator` 를 사용해 테스트할 때, 깜빡하고 테이블 이름 끝에  `_test` 를 붙이지 않고 DAG을 실행하는 경우가 존재합니다. 이런 경우 실제 운영되는 테이블을 덮어쓰거나 과거 데이터를 날려버리는 경우도 있을 수 있습니다. 운영 환경과 격리되어 안전하게 테스트할 수 있도록 별도의 개발 환경이 필요했습니다.
...
2019년~2020년, 2년 역시 계속 회사와 데이터 조직은 빠르게 성장했습니다. 데이터 그룹의 인원은 2018년 기준 7명에서 2020년 기준 약 27명으로 지속적인 성장을 하고 있었습니다. 이런 성장에 맞춰 다양한 Airflow의 DAG이 생기게 되었고, 그 당시 약 600개가 넘는 DAG을 운영하고 있었습니다. 또한 데이터 그룹의 데이터 분석가 모두 Airflow DAG을 작성할 수 있었기 때문에 DAG을 작성하는 사람들도 많아지고, 사용하는 `Operator`도 더 다양해졌습니다. 더 많은 리소스가 필요하게 되었고, 관리할 포인트가 점점 더 생겼습니다.
...
- 1) 단일 컴퓨팅이 아닌 **Kubernetes를 도입**합니다.
...
Pod 이름을 `{dag_id}.{task_id}-hashvalue` 같은 형태로 바꾸어 더 보기 쉽게 만들고 싶었습니다. 예를 들면 다음과 같은 방식으로 만들고 싶었습니다.
...
Airflow를 Kubernetes로 옮기며, 그 환경에 알맞게 사용하는 큰 흐름은 마무리지었습니다.  
...
그동안 데이터 그룹에서 Airflow 환경을 구축하며 운영에 지속해서 노력하신 토마스, 제프, 플래시, 녹스, 하디, 험프리, 우민, 카일 모두 감사합니다. 앞으로도 잘 부탁드립니다! 
