...
FMS 프로젝트에서 신뢰성 높은 데이터를 유지하기 위해서 크게 아래와 같은 검사 유형들을 나눴습니다.

1. 원본 데이터(차량 단말기 발생)에 대한 이상 여부 검사

2. 데이터 마트 테이블의 정합성 검사

3. 데이터 마트 테이블의 무결성 검사

따라서 아래와 같이 검사 시트를 만들었습니다.

데이터의 정합성과 무결성을 검사하기 위해선 이해관계자들이 함께 논의를 하는 것이 중요합니다.
...
-   프로세스
    -   각 검사 Dag들이 수행된다.
    -   검사 결과가 RDS에 저장된다
    -   Grafana에서 RDS를 통해 대시보드 만들고 Aelrt하기
-   구현
    -   SQLOperator와 정합성 검사를 통합하기 위한 Custom Operator 개발
