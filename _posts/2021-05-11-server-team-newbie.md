...
- [1년 동안 성장하게 된 점](#1년-동안-성장하게-된-점)
  ...
  - [서버에 대한 이해 - 서버를 배포하기 위한 여러 설정들 🤯](#서버에-대한-이해---서버를-배포하기-위한-여러-설정들-)
...
### 서버에 대한 이해 - 서버를 배포하기 위한 여러 설정들 🤯

두 번째는 서버에 대한 깊은 이해가 생겼는데요.

이전에는 서버를 배포하기 위한 지식이 많이 부족했었고, 이해가 많이 낮았었어요 😢

하지만 쏘카 서버팀에서는 서버 코드만을 작성하는 것이 아니라, **작업에 따라서 배포에 필요한 설정 파일들 부터, 배포 파이프라인까지 모두 공부하고, 다뤄볼 수 있기 때문에** 더욱 깊이 이해할 수 있었습니다.

저도 최근 MSA 작업을 하나 맡아 진행하면서, 해당 서버를 새롭게 띄우기 위해 `k8s` 과 `helm`, `AWS EC2 인스턴스 구성` 등등 다양한 설정들을 직접해보게 되어서 정말 좋았어요 👍
