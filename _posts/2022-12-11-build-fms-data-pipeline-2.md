...
더불어 실시간, 준-실시간으로 데이터를 조회하며 하며 정확하게 분석/집계된 데이터도 필요로 하기 떄문에 다양한 형태의 데이터를 안전하게 ...
...
**PoC 단계에서는 내부 구현이 변경될 확률이 높다는 점과 데이터 파이프라인을 구성하기 위한 명확한 요구사항이 있다는 점에서 다른 테스트 보단 E2E 테스트를 중심으로 테스트 환경을 구축하였습니다.**

## 2. 견고한 파이프라인을 위한 E2E 테스트 환경 구축하기

1편에 소개드린 주요 컴포넌트(Kafka Connect, Lambda 등)을 안정적으로 운영하기 위해 적용한 E2E 테스트에 대해 설명드리겠습니다.
